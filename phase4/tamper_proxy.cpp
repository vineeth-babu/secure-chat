/*
 * tamper_proxy.cpp -- tamper-detection demo
 */
...
/*
 * Forwards length-prefixed frames from `in` to `out`, corrupting one byte
 * of the chosen frame if this is the direction being tampered with.
 */
static void pump(int in, int out, Direction dir, std::atomic<bool> *running)
{
    ...
    while (running->load()) {
        ...
        if (dir.tamper_here && frame_index == dir.tamper_frame) {
            ...
            // so offset 8 is the first ciphertext byte
