...
{
    std::string init, ack;
    return initiator.start(init_peer, init) == e2e::Result::Ok &&
           responder.handle_init(resp_peer, init, ack) == e2e::Result::Ok &&
...
static void test_phase5_rotation()
{
    ...
    unsigned char k0a[e2e::KEY_BYTES], k0b[e2e::KEY_BYTES];
    bool got = alice.get_key("bob", k0a) && bob.get_key("alice", k0b);
    check("initial keys match", got &&
          CRYPTO_memcmp(k0a, k0b, e2e::KEY_BYTES) == 0);
    check("rotation counter is 1 after establishment",
    ...
    got = alice.get_key("bob", k1a) && bob.get_key("alice", k1b);
    check("after rotation 1 both peers agree", got &&
          CRYPTO_memcmp(k1a, k1b, e2e::KEY_BYTES) == 0);
    check("rotation 1 key DIFFERS from the initial key",
          CRYPTO_memcmp(k1a, k0a, e2e::KEY_BYTES) != 0);
    check("fingerprints match after rotation 1",
          alice.fingerprint_of("bob") == bob.fingerprint_of("alice"));
    ...
    got = alice.get_key("bob", k2a) && bob.get_key("alice", k2b);
    check("after rotation 2 both peers agree", got &&
          CRYPTO_memcmp(k2a, k2b, e2e::KEY_BYTES) == 0);
    check("rotation 2 key differs from rotation 1",
          CRYPTO_memcmp(k2a, k1a, e2e::KEY_BYTES) != 0);
    check("rotation 2 key differs from the initial key",
    ...
    check("message sent immediately after rotation decrypts correctly",
          ...
          bob.open_for_peer("alice", ct, pt) && pt == "post-rotation message");

    OPENSSL_cleanse(k0a, sizeof(k0a)); OPENSSL_cleanse(k0b, sizeof(k0b));
    OPENSSL_cleanse(k1a, sizeof(k1a)); OPENSSL_cleanse(k1b, sizeof(k1b));
...
static void test_phase5_continuity()
{
    ...
    check("baseline session established",
          rotate(alice, "bob", bob, "alice"));
    ...
    unsigned char k_mid[e2e::KEY_BYTES];
    check("alice still on the old key while the rotation is pending",
    ...
    unsigned char k_bob_new[e2e::KEY_BYTES];
    ...
    unsigned char kna[e2e::KEY_BYTES], knb[e2e::KEY_BYTES];
    bool got = alice.get_key("bob", kna) && bob.get_key("alice", knb);
    check("both peers agree on the rotated key", got &&
          CRYPTO_memcmp(kna, knb, e2e::KEY_BYTES) == 0);

    std::string ct2, pt2;
    check("a message under the new key decrypts",
    ...
    e2e::seal(k_old, "two generations old", stale_ct);
    check("two-generations-old key NO LONGER decrypts (one generation only)",
          !bob.open_for_peer("alice", stale_ct, stale_pt));
    check("no plaintext leaked from the rejected stale message",
          stale_pt.empty());

    OPENSSL_cleanse(k_old, sizeof(k_old));
    OPENSSL_cleanse(k_mid, sizeof(k_mid));
    OPENSSL_cleanse(k_bob_new, sizeof(k_bob_new));
    OPENSSL_cleanse(kna, sizeof(kna));
...
static void test_phase5_simultaneous_rotation()
{
    ...
    for (int order = 0; order < 2; order++) {
        ...
        e2e::Result ra, rb;
        if (order == 0) {
            ra = alice.handle_init("bob", ib, aa);
            rb = bob.handle_init("alice", ia, ab);
        } else {
            rb = bob.handle_init("alice", ia, ab);
            ra = alice.handle_init("bob", ib, aa);
        }

        check("tie-break: 'alice' wins and ignores bob's INIT",
              ra == e2e::Result::GlareIgnored);
        check("tie-break: 'bob' answers alice's INIT", rb == e2e::Result::Ok);

        if (rb == e2e::Result::Ok)
            check("winner completes on the loser's ACK",
                  alice.handle_ack("bob", ab) == e2e::Result::Ok);

        unsigned char ka[e2e::KEY_BYTES], kb[e2e::KEY_BYTES];
        bool got = alice.get_key("bob", ka) && bob.get_key("alice", kb);
        check("both peers still have a usable key", got);
        check("SIMULTANEOUS ROTATION CONVERGES on one identical key",
              got && CRYPTO_memcmp(ka, kb, e2e::KEY_BYTES) == 0);
        check("fingerprints agree after simultaneous rotation",
              alice.fingerprint_of("bob") == bob.fingerprint_of("alice"));
        check("the rotated key differs from the pre-rotation key",
              got && CRYPTO_memcmp(ka, k_before, e2e::KEY_BYTES) != 0);
        ...
        check("chat works after simultaneous rotation",
              ...
              bob.open_for_peer("alice", ct, pt) && pt == "after glare rotation");

        OPENSSL_cleanse(k_before, sizeof(k_before));
        OPENSSL_cleanse(ka, sizeof(ka));
        OPENSSL_cleanse(kb, sizeof(kb));
    }
}
...
int main()
{
    ...
    test_phase5_rotation();
    test_phase5_continuity();
    test_phase5_simultaneous_rotation();
