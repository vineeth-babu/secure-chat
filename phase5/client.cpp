...
// Log key rotation events with timestamp and new fingerprint
...
// Background timer thread to periodically rotate keys for active sessions
static void rekey_timer(int socket_fd,
                        crypto::SecureChannel *ch,
                        e2e::E2EManager *mgr,
                        std::atomic<bool> *running)
{
    const int ROTATE_SECONDS = 60;
        ...
        if (line.rfind("FROM|", 0) == 0) {
            ...
            if (separator != std::string::npos) {
                ...
                if (message.rfind(e2e::TAG_MSG, 0) == 0) {
                    ...
                    // Attempt decryption with fallback for messages in flight during rotation
                    std::string plain;
...
int main(int argc, char *argv[])
{
    ...
    // Set up chat session and worker threads
