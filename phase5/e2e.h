...
/*
 * PHASE 5: ONE-GENERATION TRANSITION KEY (receive-only).
 *
 * Rotation reuses the existing __E2E_INIT__ / __E2E_ACK__ handshake, which
 * §1.4 explicitly sanctions ("initiation (or re-initiation)"). But the two
 * sides do not switch keys at the same instant: the responder installs the
 * new key when the INIT arrives, while the initiator keeps using the old key
 * until the ACK comes back. A chat message sent in that window is sealed
 * under the old key and arrives after the responder has already rotated, so
 * it would fail to decrypt -- breaking §6.1's requirement that messages sent
 * "shortly before, during, and after a rotation" remain readable.
 *
 * The minimum fix is to keep EXACTLY ONE previous key, for DECRYPTION ONLY,
 * as a transition key:
 *
 *   - the current key is always the one used to ENCRYPT;
 *   - the transition key is NEVER used to encrypt;
 *   - it is not an active session key and is not reported by get_key(),
 *     is_established() or state_of();
 *   - on receive, the current key is tried first and the transition key only
 *     if authentication fails;
 *   - exactly one generation is kept: it is overwritten at the next rotation
 *     and wiped on clear()/reset(). No key history, no epochs, no queues.
 *
 * READING OF §6.1. The assignment says the old key "must be discarded and
 * never reused once the new key is confirmed". We read "reused" as reused for
 * ENCRYPTION, which is the only reading compatible with the same section's
 * continuity requirement -- if in-flight ciphertext became undecryptable the
 * continuity clause could not be satisfied at all. The retention is bounded
 * to one generation and to the receive path.
 */
...
public:
    ...
    /* ============================================================== */
    /* Phase 5: forward secrecy via periodic rekeying                 */
    /* ============================================================== */

    /*
     * Decrypt an __E2E_MSG__ payload for `peer`, trying the current key and,
     * only if that fails authentication, the one-generation transition key
     * (see the note on prev-key retention below).
     *
     * This is the ONLY place the transition key is ever consulted, and it is
     * consulted for DECRYPTION ONLY. Returns false if the peer has no usable
     * key or if neither key authenticates the payload -- in which case no
     * plaintext is produced and the caller discards the message.
     */
    ...
    /*
     * Peers whose E2E session is due for automatic rotation: they have a
     * usable key, have NO handshake already pending, and their last key
     * install was at least `seconds` ago.
     *
     * The "no pending handshake" condition is what stops the timer thread
     * from starting a second rotation for a peer while one is in flight.
     */
    std::vector<std::string> peers_due_for_rotation(int seconds);

    /* How many times this peer's key has been installed (1 = initial
       establishment, 2 = after the first rotation, ...). Used only for the
       [E2E-ROTATE] evidence line. */
